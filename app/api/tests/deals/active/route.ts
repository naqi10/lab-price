...
const KNOWN_PROFILE_CODES = Array.from(
  new Set(
    [
      ...cdlSeedData.profiles.map((p) => p.code?.toUpperCase()).filter(Boolean),
      ...qcSeedData
        .filter((p) => /^profil\b/i.test(p.name) || (p.componentCodes?.length ?? 0) > 0)
...
function normalizeSourceLabCode(code: string | null | undefined): string | null {
  if (!code) return null;
  ...
  try {
    ...
    const existingKeys = new Set(
      deals
        ...
        .map((d) => `${normalizeSourceLabCode(d.sourceLabCode)}:${d.profileCode}`)
    );
    ...
    const fallbackDeals = activeProfileTests
      ...
      .map((t, idx) => ({
        ...
        sourceLabCode: normalizeSourceLabCode(t.priceList!.laboratory.code),
        profileCode: t.code,
    ...
    const profileByLabAndCode = new Map(
      activeProfileTests
        ...
        .map((t) => [`${normalizeSourceLabCode(t.priceList!.laboratory.code)}:${t.code!}`, t] as const)
    );

    const allDeals = [...deals, ...fallbackDeals].map((deal) => {
      const normalizedSourceLabCode = normalizeSourceLabCode(deal.sourceLabCode);
      ...
      if (resolved.length > 0) {
        return { ...deal, sourceLabCode: normalizedSourceLabCode, testMappingIds: resolved };
      }
      ...
      if (deal.testMappingIds.length === 0 && deal.profileCode && normalizedSourceLabCode) {
        const seedTest = profileByLabAndCode.get(`${normalizedSourceLabCode}:${deal.profileCode}`);
        if (seedTest?.testMappingEntry?.testMappingId) {
          logger.warn({ profileCode: deal.profileCode, labCode: normalizedSourceLabCode }, "[deals/active] bundle has no resolved components, using self-mapping");
          return { ...deal, sourceLabCode: normalizedSourceLabCode, testMappingIds: [seedTest.testMappingEntry.testMappingId] };
        }
      }
      return { ...deal, sourceLabCode: normalizedSourceLabCode };
    });
    ...
    return NextResponse.json({
      ...
      data: enriched,
      meta: {
        storedActiveBundleCount: deals.length,
        returnedBundleCount: enriched.length,
      },
    });
  } catch (error) {
