...
export default function QuotationEmailHistory({
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        a.href = url;
        a.download = `devis-${quotation?.quotationNumber || quotationId}.pdf`;
        a.click();
  ...
  if (!quotation?.emails || quotation.emails.length === 0) {
    return (
      <div className="rounded-lg border border-border/40 bg-card/50 p-6 text-center">
        <p className="text-sm text-muted-foreground">Aucun email envoyé pour ce devis</p>
      </div>
    );
  }
    ...
    <TooltipProvider>
      <div className="space-y-4">
        <Table>
          ...
          <TableBody>
            {quotation.emails.map((email: any) => {
          ...
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="space-y-3">
                <h3 className="font-semibold text-sm">Détails du devis</h3>
                <div className="bg-muted/30 rounded-lg p-4 space-y-3">
                  {/* Header Info */}
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs text-muted-foreground">Numéro de devis</p>
                      <p className="text-sm font-medium">{quotation?.quotationNumber}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Laboratoire</p>
                      <p className="text-sm font-medium">{quotation?.laboratory?.name}</p>
                    </div>
                  </div>

                  {/* Customer Info */}
                  {(quotation?.customer?.name || quotation?.clientName) && (
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium">
                          {quotation?.customer?.email || quotation?.clientEmail}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {/* Items Table */}
                  <div className="mt-4">
                    <p className="text-xs text-muted-foreground mb-2">Analyses</p>
                    <div className="border border-border/40 rounded overflow-hidden">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-border/40 bg-muted/50">
                            <th className="px-3 py-2 text-left text-xs font-medium">N°</th>
                            <th className="px-3 py-2 text-left text-xs font-medium">Analyse</th>
                            <th className="px-3 py-2 text-left text-xs font-medium">Code</th>
                            <th className="px-3 py-2 text-right text-xs font-medium">Prix</th>
                          </tr>
                        </thead>
                        <tbody>
