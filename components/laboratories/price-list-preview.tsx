...
export default function PriceListPreview({
  ...
  onClose: () => void;
  onConfirm: () => void;
  data: { headers: string[]; rows: string[][]; totalRows: number } | null;
  isLoading?: boolean;
  error?: string | null;
}) {
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-auto">
        <DialogHeader>
          <DialogTitle>Aperçu de la liste de prix</DialogTitle>
        </DialogHeader>
        {error && (
        ...
        <DialogFooter>
          ...
          <Button onClick={onConfirm} disabled={isLoading || !data || !!error}>
